...
const createEducation = async (req, res) => {
    try {
        ...
        if (!id_side_effects || (!Array.isArray(id_side_effects) || id_side_effects.length === 0)) {
            return res.status(400).json({
                success: false,
                code: "BAD_REQUEST",
                error: {
...
const updateEducation = async (req, res) => {
    try {
        ...
        if (id_side_effects && Array.isArray(id_side_effects) && id_side_effects.length > 0) {
            ...
            const existingSideEffectIds = existingRecommendations.map(rec => rec.id_side_effect);
        
            // Filter out the new side effects that do not already exist
            const newSideEffects = id_side_effects.filter(id_side_effect => !existingSideEffectIds.includes(id_side_effect));
            const deletedSideEffects = existingSideEffectIds.filter(exist_side_effect => !id_side_effects.includes(exist_side_effect));
        ...
        if (thumbnail_base64 && !remove_thumbnail) {
            ...
            if (education.thumbnail) {
                const oldFilePath = path.join(__dirname, '../..', 'uploads', 'thumbnails', path.basename(education.thumbnail));
