...
const getRecomendation = async (req, res) => {
    try {
        ...
        let { status, id_user } = req.body && req.body.filter ? req.body.filter : 
        {
            status: 'active'
        };
        ...
        if (getUserSideEffectID && getUserSideEffectID.length) {
            getUserSideEffectID = (await getUserSideEffectID).map(sideEffect => sideEffect.id_user_side_effect)
        ...
        const {
            count,
            rows
        } = await Recomendation.findAndCountAll({
            ...
            include: [{
                ...
                attributes: ['id_education', 'title', 'content',
                    'video_link', 'thumbnail',
                    'status'
            ...
            },
            {
                model: SideEffects,
                as: 'side_effect',
                attributes: ['id_side_effect', 'effect_name', 'effect_detail', 'status']
            }
        ]
        });
